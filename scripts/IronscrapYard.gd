...
func _ready() -> void:
	...
	if GameManager.is_night_raid:
		world_darkness.color = Color(0.2, 0.19, 0.17, 1).lerp(Color(0.045, 0.04, 0.035, 1), darkness)
		Notify.show_toast("Night Raid (%s) - the scrapyard swallows the light out here." % GameManager.format_hour(GameManager.selected_raid_hour))
	else:
		world_darkness.color = Color(0.66, 0.62, 0.56, 1).lerp(Color(0.4, 0.37, 0.33, 1), darkness)
