...
# Main Menu - a low detuned drone, a slow sub-bass "heartbeat" pulse, and a
# slow melodic arpeggio pad. No external audio files - everything is
# generated live.
...
func _fill_buffer() -> void:
	...
	for i in range(to_fill):
		...
		var mixed := tanh((drone + pulse + arp) * 1.3)
