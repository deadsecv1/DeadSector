...
func _check_spike_damage(delta: float) -> void:
	...
	if global_position.distance_to(player.global_position) <= spike_radius + 16.0:
		player.take_damage(spike_damage, "SPIKE", "Spike Aura", global_position - player.global_position)
...
func _shoot() -> void:
	...
	# muzzle's forward offset. This matters especially here since
	# _hold_distance() deliberately closes Spike to well inside that
	# muzzle offset for its spinning-ring melee mechanic.
...
func die() -> void:
	...
	GameManager.record_kill("Spike", _last_damage_weapon)
