...
func _ready() -> void:
	...
	GameManager.notify_event("journey_to_boneclock")
	_spawn_recruit()
	_spawn_raid_party()
	_spawn_pet()
	...
	if GameManager.is_night_raid:
		world_darkness.color = Color(0.18, 0.18, 0.22, 1).lerp(Color(0.04, 0.04, 0.06, 1), darkness)
		Notify.show_toast("Night Raid (%s) - the streets are worse in the dark." % GameManager.format_hour(GameManager.selected_raid_hour))
	else:
		world_darkness.color = Color(0.6, 0.6, 0.65, 1).lerp(Color(0.38, 0.38, 0.42, 1), darkness)
