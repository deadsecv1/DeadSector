...
# Milestones panel (#46) - same shape as BattlePassPanel.gd (full-screen,
# progress bar + tier list of claimed/next/locked rows), but a smaller
# hand-authored, permanent (non-limited-time) track themed around named
# career milestones rather than a procedurally-generated 200-tier grind.
...
@onready var stones_label: Label = $VBox/TopRow/StonesLabel
...
func _ready() -> void:
	...
	sparkles.particle_color = Color(0.95, 0.8, 0.35, 1)
...
func _make_tier_row(tier: int, reward: Dictionary) -> Control:
	var claimed: bool = tier <= GameManager.milestone_tier
	var is_next: bool = tier == GameManager.milestone_tier + 1
	...
	if claimed:
		...
		sb.bg_color = Color(0.16, 0.14, 0.08, 0.9)
	...
	name_lbl.text = str(reward.get("name", "Milestone %d" % tier))
	name_lbl.add_theme_font_size_override("font_size", 14)
	name_lbl.add_theme_color_override("font_color", Color(0.95, 0.85, 0.5, 1))
