...
# The standalone Main Menu Guild button - same idle-glow/permanent-border/
# twinkling-sparkles treatment as ArenaButton.gd/MilestonesButton.gd,
# recolored to the Guild panel's own lavender accent (0.85, 0.65, 1.0).
# Guild was previously only reachable via Main Menu -> Social -> Guild;
# this is a direct entry point, that older path stays as a second way in.
...
func _ready() -> void:
	...
	glow.color = Color(0.4, 0.3, 0.5, IDLE_GLOW_ALPHA)
	...
	sb.border_color = Color(0.85, 0.65, 1.0, 0.85)
	...
	stars.star_color = Color(0.85, 0.65, 1.0, 1.0)
	stars.star_count = 4
	stars.min_size = 2.0
	stars.max_size = 3.5
	add_child(stars)
...
func _draw() -> void:
	...
	for i in range(sparkles.size()):
		...
		draw_circle(pos, sp["r"] * flicker * (0.75 if not hovering else 1.0), Color(0.85, 0.65, 1.0, 0.6 * flicker * mult))
