...
# The "Bloodline" button - sits at an angle (mirrored, tilts the other
# way), pulses continuously, and always has a faint ambient red glow +
# drifting sparkles even when not hovered, intensifying further on
# hover (matches the Spectral Tide button's treatment).
...
# "Dead Sector's Side Scroller" is much longer than the old "Mini Games!"
# idle text, so it needs a smaller, wrapped font to actually fit this
# button's fixed 160x48 box - "Bloodline" on hover is short enough to
# stay at the original size.
const IDLE_FONT_SIZE := 10
...
func _ready() -> void:
	...
	add_theme_font_size_override("font_size", IDLE_FONT_SIZE)
	glow.color = Color(0.9, 0.1, 0.15, IDLE_GLOW_ALPHA)
...
func _on_hover_start() -> void:
	hovering = true
	text = "Bloodline"
	add_theme_font_size_override("font_size", HOVER_FONT_SIZE)
	Sfx.play_blood_hover()
	var tw := create_tween()
...
func _on_hover_end() -> void:
	hovering = false
	text = "Dead Sector's Side Scroller"
	add_theme_font_size_override("font_size", IDLE_FONT_SIZE)
	var tw := create_tween()
...
func _draw() -> void:
	...
	for i in range(sparkles.size()):
		...
		var col: Color = Color(0.15, 0.03, 0.04, 0.7 * flicker * mult) if sp["dark"] else Color(0.9, 0.15, 0.2, 0.6 * flicker * mult)
		draw_circle(pos, sp["r"] * flicker * (0.75 if not hovering else 1.0), col)
