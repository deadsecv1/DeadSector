...
func test_controller_mapping_matches_the_actual_runtime_bindings() -> void:
	...
	assert_eq(GameManager.JOYPAD_BUTTON_BINDINGS["nightvision"], JOY_BUTTON_X, "Settings' reference text claims Nightvision is X")
	assert_eq(GameManager.JOYPAD_BUTTON_BINDINGS["reload"], JOY_BUTTON_X, "Settings' reference text claims Reload shares X with Nightvision")
